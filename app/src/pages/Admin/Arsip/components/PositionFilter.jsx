...
export default function PositionFilter({ value = "all", positions = [], onChange }) {
  const options = [
      ...
      {({ open }) => (
        <div className="relative w-48">
          <Listbox.Button className="w-full flex justify-between items-center px-3 py-2 border border-gray-300 rounded-lg bg-white shadow-sm text-left text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/30">
            ...
            <ChevronDown
              className={`w-4 h-4 text-gray-500 ml-1 shrink-0 transition-transform duration-200 ${
                open ? "rotate-180" : ""
              }`}
            />
          </Listbox.Button>
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute mt-2 w-full max-h-64 overflow-y-auto bg-white border border-gray-200 rounded-lg shadow-lg z-10 text-sm p-1">
              {options.map((opt) => (
                <Listbox.Option key={opt.value} value={opt.value}>
                  {({ active, selected }) => (
                    ...
                    >
                      ...
                      {selected && <Check className="w-4 h-4 text-sky-600 shrink-0" />}
                    </div>
                  )}
                </Listbox.Option>
              ))}
