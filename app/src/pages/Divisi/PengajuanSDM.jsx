...
export default function PengajuanSDM() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.message || "Gagal mengirim pengajuan. Coba lagi.",
      );
    } finally {
      setSubmitting(false);
    }
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-3 py-2.5 rounded-lg border text-sm placeholder:text-gray-400 focus:outline-none ${
                    deptLocked
                      ? "border-gray-200 bg-gray-50 text-gray-500 cursor-not-allowed pr-9"
              ...
              {deptLocked && (
                <p className="text-xs text-gray-400 mt-1">
                  Otomatis terisi sesuai akun divisi Anda.
                </p>
              )}
            </div>
                  ...
                  <div key={idx} className="flex items-center gap-2">
                    <input
                      ...
                      placeholder={`${idx + 1}. Tanggung jawab ${
                        ["pertama", "kedua", "ketiga"][idx] ?? "lainnya"
                      }...`}
